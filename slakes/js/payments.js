...
function clickSave() {
	$("button#save_payment").click(function () {
		if ($("input#num_lessons").val().trim() == '' || $("input#total").val().trim() == '') {
		...
		$.ajax({
			...
			data : {
				...
				'num_lessons' : $('input#num_lessons').val().trim(),
				'total' : $('input#total').val().trim()
			},
			success : function () {
				alert('saved');
				$("div#edit").addClass("hidden");
